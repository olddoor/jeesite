...
<body>
    ...
    <div class="row">
      <div class="span4">
		...
		<!-- 获取组织结构栏目下面的文章对象集合, 通过tld的配置调用CmsUtil.getArticleList方法从db获取 -->
		<ul><c:forEach items="${fnc:getArticleList(site.id, 2, 8, '')}" var="article">
			...
			${article.url}<a href="${article.url}" style="color:${article.color}">${fns:abbr(article.title,28)}</a></li>
		</c:forEach></ul>
      </div>
      <div class="span4">
		...
		<ul><c:forEach items="${fnc:getArticleList(site.id, 6, 8, '')}" var="article">
			<li><span class="pull-right"><fmt:formatDate value="${article.updateDate}" pattern="yyyy.MM.dd"/></span>
			${urlSuffix}值为urlSuffix=.html
			<a href="${article.url}" style="color:${article.color}">${fns:abbr(article.title,28)}</a></li>
		</c:forEach></ul>
      </div>
      <div class="span4">
