...
<html>
<head>
	<title>cms 装饰页面<sitemesh:title default="欢迎光临"/> - ${site.title} - Powered By JeeSite</title>
...
<body>
        ...
        <div class="container">
          <c:choose>
   			<c:when test="${not empty site.logo}">
   				img:<img alt="${site.title}" src="${site.logo}" class="container" onclick="location='${ctx}/index-${site.id}${fns:getUrlSuffix()}'">
   			</c:when>
   			<c:otherwise><a class="brand" href="${ctx}/index-${site.id}${fns:getUrlSuffix()}">brand: ${site.title}</a></c:otherwise>
   		  </c:choose>
	...
	<div class="container">
		...
		<footer>
			...
			<div class="pull-right">${fns:getDate('yyyy年MM月dd日 E')}</div><div class="copyright">${site.copyright}</div>who are u~
      	</footer>
    </div> <!-- /container -->
</body>
</html>